...
pub const TlsInit = struct {
    pub const Result = union(enum) {
        ...
        action: union(enum) {
            writevAll: []std.posix.iovec_const,
            decoder: union(enum) {
                readAtLeastOurAmt: struct { decoder: *tls.Decoder, our_amt: usize },
                readAtLeast: struct { decoder: *tls.Decoder, their_amt: usize },
            },
        },
    };

    pub fn RunArg(E: type) type {
        return union {
            options: Options,
            stream: union {
                writevAll: E!void,
                decoder: union {
                    readAtLeastOurAmt: E!void,
                    readAtLeast: E!void,
                },
            },
        };
    ...
    key_seq: u64 = 0,
    client_hello_rand: [32]u8 = undefined,
    server_hello_rand: [32]u8 = undefined,
    random_buffer: [176]u8 = undefined,
    legacy_session_id: [32]u8 = undefined,
    key_share: KeyShare = undefined,
    cleartext_header_buf: [1600]u8 = undefined,
    cleartext_header: []const u8 = undefined,
    d: tls.Decoder = undefined,
    tls_version: tls.ProtocolVersion = undefined,
    ...
    //   next one.
    cert_buf_index: usize = 0,
    write_seq: u64 = 0,
    read_seq: u64 = 0,
    prev_cert: Certificate.Parsed = undefined,
    pending_cipher_state: CipherState = .cleartext,
    cipher_state: CipherState = .cleartext,
    handshake_state: HandshakeState = .hello,
    handshake_cipher: tls.HandshakeCipher = undefined,
    main_cert_pub_key: CertificatePublicKey = undefined,
    now_sec: i64 = undefined,
    cleartext_fragment_start: usize = 0,
    ...
    pub fn run(self: *@This(), stream: anytype, E: type, arg: RunArg(E)) InitError(@TypeOf(stream))!Result {
        outer: switch (self.state) {
            .CleartextHeader => {
                ...
                self.host = switch (self.options.host) {
                    .no_verification => "",
                    .explicit => |host| host,
                };
                const host_len: u16 = @intCast(self.host.len);

                crypto.random.bytes(&self.random_buffer);
                self.client_hello_rand = self.random_buffer[0..32].*;
                self.legacy_session_id = self.random_buffer[32..64].*;

                self.key_share = KeyShare.init(self.random_buffer[64..176].*) catch |err| switch (err) {
                ...
                const extensions_payload = tls.extension(.supported_versions, array(u8, tls.ProtocolVersion, .{
                    ...
                    int(u16, @intFromEnum(tls.NamedGroup.x25519_ml_kem768)) ++
                        array(u16, u8, self.key_share.ml_kem768_kp.public_key.toBytes() ++ self.key_share.x25519_kp.public_key) ++
                        int(u16, @intFromEnum(tls.NamedGroup.secp256r1)) ++
                        array(u16, u8, self.key_share.secp256r1_kp.public_key.toUncompressedSec1()) ++
                        int(u16, @intFromEnum(tls.NamedGroup.secp384r1)) ++
                        array(u16, u8, self.key_share.secp384r1_kp.public_key.toUncompressedSec1()) ++
                        int(u16, @intFromEnum(tls.NamedGroup.x25519)) ++
                        array(u16, u8, self.key_share.x25519_kp.public_key),
                ));
                const server_name_extension = int(u16, @intFromEnum(tls.ExtensionType.server_name)) ++
                    ...
                    .{0x00} ++ // name_type
                    int(u16, host_len);
                const server_name_extension_len = switch (self.options.host) {
                    .no_verification => 0,
                ...
                const client_hello =
                    ...
                    [1]u8{32} ++ self.legacy_session_id ++
                    cipher_suites ++
                ...
                self.cleartext_header_buf = .{@intFromEnum(tls.ContentType.handshake)} ++
                    ...
                    out_handshake;
                self.cleartext_header = switch (self.options.host) {
                    .no_verification => self.cleartext_header_buf[0 .. self.cleartext_header_buf.len - server_name_extension.len],
            ...
            .PreFragment => {
                ...
                self.now_sec = std.time.timestamp();
                self.d = .{ .buf = &self.handshake_buffer };
                self.state = .Fragment;
            ...
            .HandshakePreLoopStart => {
                ...
                self.ctd, self.ct = content: switch (self.cipher_state) {
                    ...
                    .handshake => {
                        ...
                        self.read_seq += 1;
                        self.cleartext_fragment_end -= 1;
                        const ct: tls.ContentType = @enumFromInt(cleartext_buf[self.cleartext_fragment_end]);
                        if (ct != .handshake) return error.TlsUnexpectedMessage;
                        break :content .{ tls.Decoder.fromTheirSlice(@constCast(cleartext_buf[self.cleartext_fragment_start..self.cleartext_fragment_end])), ct };
                    },
                    .application => {
                        ...
                        switch (self.handshake_cipher) {
                            inline else => |*p| {
                                ...
                                self.cleartext_fragment_end += message_len;
                            },
                        }
                        self.read_seq += 1;
                ...
                switch (self.ct) {
                    .alert => {
                        self.ctd.ensure(2) catch {
                            self.state = .Fragment;
                            continue :outer .Fragment;
                        };
                        const level = self.ctd.decode(tls.AlertLevel);
                        const desc = self.ctd.decode(tls.AlertDescription);
                        _ = level;
                    ...
                    .change_cipher_spec => {
                        self.ctd.ensure(1) catch {
                            self.state = .Fragment;
                            continue :outer .Fragment;
                        };
                        if (self.ctd.decode(tls.ChangeCipherSpecType) != .change_cipher_spec) return error.TlsIllegalParameter;
                        self.cipher_state = self.pending_cipher_state;
                    },
                    .handshake => {
                        self.state = .HandshakeLoopStart;
                        continue :outer .HandshakeLoopStart;
                    },
            ...
            .HandshakeLoopStart => {
                ...
                switch (handshake_type) {
                    .server_hello => {
                        ...
                        if (!hsd.eof()) {
                            ...
                            while (!all_extd.eof()) {
                                ...
                                switch (et) {
                                    ...
                                    .key_share => {
                                        if (self.key_share.getSharedSecret()) |_| return error.TlsIllegalParameter;
                                        try extd.ensure(4);
                                        const named_group = extd.decode(tls.NamedGroup);
                                        const key_size = extd.decode(u16);
                                        try extd.ensure(key_size);
                                        try self.key_share.exchange(named_group, extd.slice(key_size));
                                    },
                                    else => {},
                                }
                            }
                        ...
                        self.tls_version = @enumFromInt(supported_version orelse legacy_version);
                        switch (self.tls_version) {
                            .tls_1_3 => if (!mem.eql(u8, legacy_session_id_echo, &self.legacy_session_id)) return error.TlsIllegalParameter,
                            .tls_1_2 => if (mem.eql(u8, self.server_hello_rand[24..31], "DOWNGRD") and
                                self.server_hello_rand[31] >> 1 == 0x00) return error.TlsIllegalParameter,
                        ...
                        switch (cipher_suite_tag) {
                            ...
                            => |tag| {
                                self.handshake_cipher = @unionInit(tls.HandshakeCipher, @tagName(tag.with()), .{
                        ...
                        switch (self.tls_version) {
                            .tls_1_3 => {
                                switch (cipher_suite_tag) {
                                    ...
                                    .AEGIS_128L_SHA256,
                                    => |tag| {
                                        const sk = self.key_share.getSharedSecret() orelse return error.TlsIllegalParameter;
                                        const p = &@field(self.handshake_cipher, @tagName(tag.with()));
                                        const P = @TypeOf(p.*).A;
                                ...
                                self.pending_cipher_state = .handshake;
                    ...
                    .certificate => cert: {
                        if (self.cipher_state == .application) return error.TlsUnexpectedMessage;
                        ...
                        while (!certs_decoder.eof()) {
                            ...
                            if (self.cert_index == 0) {
                                ...
                                switch (self.options.host) {
                                    .no_verification => {},
                                    .explicit => try subject.verifyHostName(self.host),
                                }

                                // Keep track of the public key for the
                                // certificate_verify message later.
                                try self.main_cert_pub_key.init(subject.pub_key_algo, subject.pubKey());
                            } else {
                                try self.prev_cert.verify(subject, self.now_sec);
                            }

                            switch (self.options.ca) {
                                .no_verification => {
                                    self.handshake_state = .trust_chain_established;
                                    break :cert;
                                },
                                .self_signed => {
                                    try subject.verify(subject, self.now_sec);
                                    self.handshake_state = .trust_chain_established;
                                    break :cert;
                                },
                                .bundle => |ca_bundle| if (ca_bundle.verify(subject, self.now_sec)) |_| {
                                    self.handshake_state = .trust_chain_established;
                                    break :cert;
                                } else |err| switch (err) {
                            ...
                            self.prev_cert = subject;
                            self.cert_index += 1;
                        }
                        self.cert_buf_index += 1;
                    },
                    .server_key_exchange => {
            ...
            .HandshakeSwitchEnd => {
                ...
                self.cleartext_fragment_start = self.ctd.idx;
                self.state = .HandshakeLoopStart;
                continue :outer .HandshakeLoopStart;
            },
            .HandshakeLoopEnd => {
                self.cleartext_fragment_start = 0;
                self.cleartext_fragment_end = 0;
                self.state = .Fragment;
                continue :outer .Fragment;
            },
